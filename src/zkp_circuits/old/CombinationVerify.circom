...
template VerifyCombination() {
    ...
    for(var i = 0; i < pins; i++){
        for(var j = 0; j < pins; j++){
            ...
            onlyCrossMatch[i][j].input2 <== blackpegs[i];
        }
    }
    ...
    for(var i = 0; i < pins; i++){
        for(var j = 0; j < pins; j++){
            alreadyPresent[i][j] = Present();
            if(j == 0)
                alreadyPresent[i][j].input1 <== j;
            else
                alreadyPresent[i][j].input1 <== alreadyPresent[i][j-1].present;
            alreadyPresent[i][j].input2 <== onlyCrossMatch[i][j].match;
        }
    }
    ...
    for(var i = 0; i < pins; i++){
        for(var j = 0; j < pins; j++){
            ...
            if(j == 0)
                singleCrossMatch[i][j].input2 <== j;
            else
                singleCrossMatch[i][j].input2 <== alreadyPresent[i][j-1].present;
        }
    }
